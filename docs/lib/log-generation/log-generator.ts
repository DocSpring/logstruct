...
export class LogGenerator extends RandomDataGenerator {
    ...
    const result: JsonRecord = (() => {
      switch (logType) {
        ...
        case LogType.ACTIVEJOB: {
          switch (evt ?? this.sample([Event.Enqueue, Event.Schedule, Event.Start, Event.Finish])) {
            case Event.Enqueue:
        ...
        case LogType.SECURITY: {
          switch (evt ?? this.sample([Event.BlockedHost, Event.CSRFViolation, Event.IPSpoof])) {
        ...
        case LogType.SHRINE: {
          switch (evt ?? this.sample([Event.Upload, Event.Download, Event.Delete])) {
            case Event.Download:
        ...
        case LogType.GOODJOB: {
          switch (
            evt ??
            this.sample([Event.Enqueue, Event.Start, Event.Finish, Event.Error, Event.Log])
          ) {
            case Event.Enqueue:
        ...
        case LogType.ACTIVEMODELSERIALIZERS:
          return ActiveModelSerializers.ActiveModelSerializersGenerate.random(this).serialize();

        case LogType.AHOY:
          return Ahoy.AhoyLog.random(this).serialize();

        case LogType.DOTENV: {
          switch (evt ?? this.sample([Event.Update, Event.Load, Event.Save, Event.Restore])) {
            case Event.Load:
          ...
          switch (evt ?? this.sample([Event.Start, Event.Shutdown])) {
            ...
            default:
              return Puma.PumaStart.random(this, {
                listening_addresses: [`tcp://127.0.0.1:${this.randomInt(3000, 3999)}`],
  ...
  private getRandomRequestParams(): Record<string, any> {
    const scenario = this.sample(['login', 'register', 'payment', 'search', 'profile_update']);
    switch (scenario) {
      case 'login':
        return {
