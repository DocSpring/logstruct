...
class LogstructTest < ActionDispatch::IntegrationTest
  ...
  test "basic logging produces structured JSON logs" do
    ...
    assert log_entries.length >= 3, "Should have at least 3 log entries"
  ...
  test "error logging produces structured error logs" do
    ...
    assert exception_entries.length >= 1, "Should have at least one exception log entry"
  ...
  test "context and tagging works correctly" do
    ...
    assert tagged_entries.any?, "Should have at least one tagged log entry"
