    ...
    Open3.popen3(env, *cmd) do |_stdin, stdout, stderr, wait_thr|
      Timeout.timeout(30) do
        while (line = stdout.gets)
          lines << line.strip
          # LogStruct outputs JSON with listening_addresses, or "Use Ctrl-C to stop" when ready
          break if line.include?("listening_addresses") || line.include?("Use Ctrl-C to stop")
        end
      end
