...
RSpec.describe RailsStructuredLogging::ActionMailer do
  ...
  describe ".setup" do
    before do
      ...
      allow(RailsStructuredLogging).to receive(:enabled?).and_return(true)
      allow(RailsStructuredLogging).to receive(:configuration).and_return(
        double("Configuration", actionmailer_integration_enabled: true)
      )
      ...
      if Rails.gem_version < Gem::Version.new("7.1.0")
        allow(described_class).to receive(:setup_callbacks_for_rails_7_0)
      end
    end
