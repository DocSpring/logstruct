...
module LogStruct::Integrations::ActionMailer
  RSpec.describe Logger do
    ...
    before do
      allow(LogStruct::Integrations::ActionMailer::MetadataCollection).to receive(:add_message_metadata)
      allow(LogStruct::Integrations::ActionMailer::MetadataCollection).to receive(:add_context_metadata)
    ...
    describe ".build_base_log_data" do
      ...
      it "calls metadata collection methods" do
        expect(LogStruct::Integrations::ActionMailer::MetadataCollection).to receive(:add_message_metadata)
        expect(LogStruct::Integrations::ActionMailer::MetadataCollection).to receive(:add_context_metadata)

        LogStruct::Integrations::ActionMailer::Logger.build_base_log_data(mailer, "test_event")
      end
    end
    ...
    describe ".log_structured_error" do
      ...
      it "logs structured error information" do
        expect(LogStruct::Integrations::ActionMailer::Logger).to receive(:build_base_log_data).with(mailer, "email_error").and_call_original
        expect(LogStruct::Integrations::ActionMailer::Logger).to receive(:log_to_rails).with(kind_of(Hash), :error)

        LogStruct::Integrations::ActionMailer::Logger.log_structured_error(mailer, error, message)
      end

      it "includes error information in log data" do
        allow(LogStruct::Integrations::ActionMailer::Logger).to receive(:log_to_rails) do |log_data, level|
    ...
    describe ".log_to_rails" do
      it "sends log message to Rails logger with correct level" do
        LogStruct::Integrations::ActionMailer::Logger.log_to_rails("test message", :info)
        expect(rails_logger).to have_received(:info).with("test message")

        LogStruct::Integrations::ActionMailer::Logger.log_to_rails("error message", :error)
        expect(rails_logger).to have_received(:error).with("error message")

        LogStruct::Integrations::ActionMailer::Logger.log_to_rails({key: "value"}, :debug)
