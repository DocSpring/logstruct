# typed: strict
# frozen_string_literal: true

# AUTO-GENERATED: DO NOT EDIT
# Generated by scripts/generate_sorbet_log_structs.rb
# Schemas dir: schemas/log_sources/
# Template: scripts/templates/generate_log_structs/event.rb.erb

require_relative "../interfaces/common_fields"
require_relative "../interfaces/additional_data_field"
require_relative "../interfaces/request_fields"
require_relative "../shared/serialize_common"
require_relative "../shared/merge_additional_data_fields"
require_relative "../shared/add_request_fields"
require_relative "../../enums/source"
require_relative "../../enums/event"
require_relative "../../enums/level"
require_relative "../../enums/log_field"

module LogStruct
  module Log
    class Puma
      class Started < T::Struct
        extend T::Sig
        include Interfaces::CommonFields
        include SerializeCommon

        include Interfaces::AdditionalDataField
        include MergeAdditionalDataFields

        const :source, Source::Puma, default: T.let(Source::Puma, Source::Puma)

        const :event, Event::Started, default: T.let(Event::Started, Event::Started)
        const :timestamp, Time, factory: -> { Time.now }
        const :level, Level, default: T.let(Level::Info, Level)

        const :additional_data, T::Hash[Symbol, T.untyped], default: {}
        const :mode, T.nilable(String), default: nil
        const :puma_version, T.nilable(String), default: nil
        const :puma_codename, T.nilable(String), default: nil
        const :ruby_version, T.nilable(String), default: nil
        const :min_threads, T.nilable(Integer), default: nil
        const :max_threads, T.nilable(Integer), default: nil
        const :environment, T.nilable(String), default: nil
        const :process_id, T.nilable(Integer), default: nil
        const :listening_addresses, T.nilable(T::Array[String]), default: nil

        sig { override.params(strict: T::Boolean).returns(T::Hash[Symbol, T.untyped]) }
        def serialize(strict = true)
          h = serialize_common(strict)
          merge_additional_data_fields(h)
          h[LogField::Mode.serialize] = mode if mode
          h[LogField::PumaVersion.serialize] = puma_version if puma_version
          h[LogField::PumaCodename.serialize] = puma_codename if puma_codename
          h[LogField::RubyVersion.serialize] = ruby_version if ruby_version
          h[LogField::MinThreads.serialize] = min_threads if min_threads
          h[LogField::MaxThreads.serialize] = max_threads if max_threads
          h[LogField::Environment.serialize] = environment if environment
          h[LogField::ProcessId.serialize] = process_id if process_id
          h[LogField::ListeningAddresses.serialize] = listening_addresses if listening_addresses
          h
        end
      end
    end
  end
end
